...
test.describe("Sprint 8 web smoke — write AI mock mode", () => {
  test("write page renders; AI button inactive; no prompt/context leak", async ({ page }) => {
...
test.describe("Sprint 8 web smoke — write AI API mode", () => {
  ...
  test.skip(
    !apiModeEnabled,
    "API mode skipped — set SMOKE_WEB_API_MODE=true and run via scripts/sprint8-smoke-web.ps1 -IncludeApiMode",
  );
  ...
  test("AI enabled inserts mock prose into editor with credit notice", async ({ page }) => {
    test.skip(
      !aiEnabled,
      "AI success skipped — restart dev:api with AI_GENERATION_ENABLED=true and AI_PROVIDER_MOCK=true",
    );
