...
/**
 * Deterministic Season → MiniArc → Chapter grouping. Chapters are chunked into
 * arcs of DEFAULT_MINI_ARC_SIZE; ranges are always valid + contiguous so chapter
 * → arc assignment never drifts. AI may rename arcs but not redraw ranges.
 */
export function buildMiniArcDrafts(chapters: ChapterDraft[]): MiniArcDraft[] {
  if (chapters.length === 0) return [];
  const sorted = [...chapters].sort((a, b) => a.chapterNumber - b.chapterNumber);
  const arcs: MiniArcDraft[] = [];
  for (let i = 0; i < sorted.length; i += DEFAULT_MINI_ARC_SIZE) {
    const slice = sorted.slice(i, i + DEFAULT_MINI_ARC_SIZE);
    const first = slice[0];
    const last = slice[slice.length - 1];
    const arcNumber = arcs.length + 1;
    arcs.push({
      arcNumber,
      title: `Arc ${arcNumber}: ${first.title}`,
      premise: (first.purpose || first.summary || "").slice(0, 600),
      startChapter: first.chapterNumber,
      endChapter: last.chapterNumber,
      payoff: last.endingHook || last.miniVictory || null,
    });
  }
  return arcs;
}

/** Overrides deterministic arc titles/premise/payoff from AI output by index (ranges untouched). */
function applyAiArcNaming(arcs: MiniArcDraft[], raw: unknown): MiniArcDraft[] {
  if (!Array.isArray(raw)) return arcs;
  return arcs.map((arc, index) => {
    const item = raw[index];
    if (!item || typeof item !== "object") return arc;
    const a = item as Record<string, unknown>;
    const title = typeof a.title === "string" && a.title.trim() ? a.title.trim().slice(0, 200) : arc.title;
    const premise =
      typeof a.premise === "string" && a.premise.trim() ? a.premise.trim().slice(0, 600) : arc.premise;
    const payoff =
      typeof a.payoff === "string" && a.payoff.trim() ? a.payoff.trim().slice(0, 400) : arc.payoff;
    return { ...arc, title, premise, payoff };
  });
}
...
export function generateOutlineDraft(
  ...
  return {
    ...
    miniArcs: buildMiniArcDrafts(chapters),
    chapters,
    openLoops,
    plannedReveals,
  };
...
function buildOutlineDraftFromAi(
  ...
  return {
    ...
    miniArcs: applyAiArcNaming(buildMiniArcDrafts(chapters), parsed.miniArcs),
    chapters,
    openLoops,
    plannedReveals,
  };
...
function buildOutlineSystemPrompt(targetCount: number): string {
  return (
    ...
    '  "miniArcs": [ { "title": "judul arc singkat", "premise": "1 kalimat fokus arc", "payoff": "1 kalimat bayaran arc" } ]\n' +
    "}\n\n" +
