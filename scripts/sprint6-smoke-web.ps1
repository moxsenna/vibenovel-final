...
.DESCRIPTION
  ...
  optional API mode (foundation + outline + write ready_for_summary bootstrap via API).

  Run from repo root:
    npm run smoke:web:summary
    npm run smoke:web:summary -- -IncludeApiMode
...
.PARAMETER IncludeApiMode
  Bootstrap foundation + outline + write ready_for_summary via API, then run API-mode Playwright summary flow.
...
function Write-SmokeHeader {
  Write-Host ""
  Write-Host "VibeNovel Sprint 6 Summary Web Smoke Test" -ForegroundColor Cyan
...
function Bootstrap-WriteReadyForSummary {
  param(
    [string]$ProjectId,
    [hashtable]$AuthHeaders
  )
  ...
  return @{ ChapterOutlineId = $ch1; SessionId = $sessionId }
}

function Invoke-PlaywrightSummary {
  param(
    [string]$Name,
    [hashtable]$ExtraEnv = @{}
  )
  ...
  try {
    $specFile = "e2e/sprint6-summary-flow.spec.ts"
    $grepArg = $ExtraEnv["SMOKE_WEB_API_MODE"]
    if ($grepArg -eq "true") {
      & npx playwright test $specFile --grep "summary API mode" 2>&1 | ForEach-Object { Write-Host $_ }
    } else {
      & npx playwright test $specFile --grep "summary mock mode" 2>&1 | ForEach-Object { Write-Host $_ }
    }
...
if (-not $SkipMockMode) {
  $mockOk = Invoke-PlaywrightSummary -Name "mock-mode Playwright (/summary)"
...
if ($IncludeApiMode) {
  ...
  if (-not $anonKey) {
    ...
    Add-StepResult "API-mode Playwright (/summary)" "NOT RUN" "missing anon key"
    $anyFail = $true
  } else {
    Add-StepResult "resolve Supabase anon key" "PASS" "source=env|cli"

    if ([string]::IsNullOrWhiteSpace($TestEmail)) {
      $TestEmail = "s6websmoke-$(Get-Random -Maximum 99999999)@example.com"
    }
    ...
    if ($token -and $apiHealthy) {
      try {
        ...
        $created = Invoke-ApiRequest -Method POST -Path "/api/projects" -Headers $auth `
          -Body '{"title":"S6 Summary Web Smoke","entryPath":"rough_idea"}'
        ...
        $writeCtx = Bootstrap-WriteReadyForSummary -ProjectId $projectId -AuthHeaders $auth
        Add-StepResult "API-mode bootstrap ready_for_summary" "PASS" "ch1=$($writeCtx.ChapterOutlineId)"
      } catch {
        Add-StepResult "API-mode bootstrap ready_for_summary" "FAIL" $_.Exception.Message
        $projectId = $null
      }
    }
    ...
    if ($token -and $projectId) {
      ...
      $apiOk = Invoke-PlaywrightSummary -Name "API-mode Playwright (/summary full flow)" -ExtraEnv $apiEnv
      if (-not $apiOk) { $anyFail = $true }
    } else {
      Add-StepResult "API-mode Playwright (/summary)" "NOT RUN" "signup, API health, or bootstrap failed"
      $anyFail = $true
    }
  }
} else {
  Add-StepResult "API-mode Playwright (/summary)" "NOT RUN" "use -IncludeApiMode"
}
...
Write-Host ""
Write-Host "CI note: summary API-mode E2E deferred from GitHub Actions (browser + Supabase + dual env)." -ForegroundColor DarkGray
